...
export const Sidebar: React.FC<SidebarProps> = ({
  ...
  return (
    <div
      className={`w-full flex flex-col h-full border-r shrink-0 transition-colors relative ${
    ...
    >
      ...
      {bottomTab === 'chats' ? (
        <div className="flex-1 overflow-y-auto divide-y divide-slate-100/80 dark:divide-slate-800/50 select-none">
                    ...
                    {!isNoteToSelf && (
                      <span
                        className={`absolute bottom-0 right-0 w-3 h-3 rounded-full border-2 ${
                          ...
                          thread.contact.status === 'online' ? 'bg-emerald-500' : 'bg-yellow-400'
                        }`}
                        title={thread.contact.status === 'online' ? 'Active / Online' : 'Offline'}
                      />
                    )}
                  </div>
